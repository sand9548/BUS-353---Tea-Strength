...
//Rare exceptions to indicate a bug in the _teastrength code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_teastrength = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_teastrength]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.teastrength.module"
